...
public class ItemOrderController extends BaseController {
    ...
    public String add(Model model){
        String sql = "select * from user where 1=1 order by id";
        List<User> l_user = userService.listBySqlReturnEntity(sql);
        model.addAttribute("types",l_user);
     ...
     * 新订单保存
     */
    @RequestMapping("/addByHand")
    public String addByHand(ItemOrder itemOrder){
    ...
    public String delete(Integer id){
        ...
        orderDetailService.deleteByEntity(orderDetailService.getBySqlReturnEntity(sql));
    ...
    public String my(Model model, HttpServletRequest request){
        ...
        //全部订单
        String sql = "select * from item_order where user_id="+userId+" and isDelete=0 order by id desc";
        List<ItemOrder> all = itemOrderService.listBySqlReturnEntity(sql);
        //待发货
        String sql2 = "select * from item_order where user_id="+userId+" and isDelete=0 and status=0 order by id desc";
        List<ItemOrder> dfh = itemOrderService.listBySqlReturnEntity(sql2);

        //已取消,后台取消或是用户取消
        String sql3 = "select * from item_order where user_id="+userId+" and isDelete=1 or status=1 order by id desc";
        List<ItemOrder> yqx = itemOrderService.listBySqlReturnEntity(sql3);
        //已发货
        String sql4 = "select * from item_order where user_id="+userId+" and isDelete=0 and status=2 order by id desc";
        List<ItemOrder> dsh = itemOrderService.listBySqlReturnEntity(sql4);
        //已收货
        String sql5 = "select * from item_order where user_id="+userId+" and isDelete=0 and status=3 order by id desc";
     ...
     * 后台强制收货
     */
    @RequestMapping("/force_sh")
    public String force_sh(Integer id,Model model){
        ItemOrder obj =itemOrderService.load(id);
        obj.setStatus(3);
        itemOrderService.updateById(obj);
        model.addAttribute("obj",obj);
