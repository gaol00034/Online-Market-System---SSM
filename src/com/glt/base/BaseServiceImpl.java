...
public abstract  class BaseServiceImpl<T> implements BaseService<T>{
	...
	public Pager<T> findBySqlRerturnEntity(String sql) {
		 ...
		 * 执行分页
		 */
    	Integer pageSize = SystemContext.getPageSize();//com.glt.Utils.SystemContext
		Integer pageOffset = SystemContext.getPageOffset();
		if(pageOffset==null||pageOffset<0) pageOffset = 0;//初始页数
		if(pageSize==null||pageSize<0) pageSize = 15;//若未设置分页数，则设置为15
		String order = SystemContext.getOrder();
		String sort = SystemContext.getSort();
		Integer pageNum = null;
		if(pageOffset == 0){
		...
		PageHelper.startPage(pageNum, pageSize);//开始分页（引用） package com.github.pagehelper;Copyright (c) 2014 abel533@gmail.com
